...
The standard public boundary check rejects UI/runtime assets. During a private extraction and before release, maintainers must also pass a non-committed boundary-policy file to `python scripts/check_boundary.py --policy /secure/path/policy.txt`; each line is either `marker VALUE` or `path-prefix VALUE`. That policy is a private release-control input, not source code or a public artifact.
